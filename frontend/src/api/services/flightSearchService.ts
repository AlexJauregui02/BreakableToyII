...
import type {
  IataCodeAirlineSearchResponse,
  IataCodeAirportSearchResponse,
} from "@/types/iataCodeSearch";

export async function getFlightOffers(
  data: FlightSearchOffer
): Promise<apiResponse<flightOfferResponse> | undefined> {
  ...
  if (data.returnDate) {
    url += `&returnDate=${data.returnDate}`;
  }
  if (data.nonStop) {
    url += `&nonStop=true`;
  }
  if (data.currencyCode) {
    url += `&currencyCode=${data.currencyCode}`;
  }
  if (data.sortBy) {
...
export async function getCityNameFromIataCode(
  iataCode: string,
  countryCode: string
): Promise<apiResponse<IataCodeAirportSearchResponse> | undefined> {
...
export async function getAirlineNameFromIataCode(
  iataCode: string
): Promise<apiResponse<IataCodeAirlineSearchResponse> | undefined> {
  let url = `/airline-information?airlineCode=${iataCode}`;

  return fetchApi<apiResponse<IataCodeAirlineSearchResponse>>(url);
}

export async function searchAirportsByKeyword(
  keyword: string
): Promise<IataCodeAirportSearchResponse[]> {
  if (!keyword) return [];
