...
export const FlightResultsProvider = ({ children }: { children: ReactNode }) => {
    ...
    const setResults = (data: apiResponse<flightOfferResponse>) => {
        setResultsState(data);
    };
    const clearResults = () => setResultsState(null);

    const setLocationsName = (data: LocationsName) => {
        setLocationsNameState(data);
    };
    const clearLocationsName = () => setLocationsNameState([]);

    const setSearchParams = (data: FlightSearchOffer) => {
        setSearchParamsState(data);
    };
